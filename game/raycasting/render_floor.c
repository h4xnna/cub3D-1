...
/*   Updated: 2026/01/08 19:58:41 by pacda-si         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
		...
		unsigned int reflected_color, unsigned int wall_color)
{
	...
	make_color_from_uint(floor_color, &floort);
	make_color_from_uint(wall_color, &wall);
	make_color_from_uint(reflected_color, &reflect);
	final.r = (uint8_t)((floort.r * FLOOR_W + reflect.r * REFLECT_W + wall.r
				* WALL_W));
	final.g = (uint8_t)((floort.g * FLOOR_W + reflect.g * REFLECT_W + wall.g
				* WALL_W));
	final.b = (uint8_t)((floort.b * FLOOR_W + reflect.b * REFLECT_W + wall.b
				* WALL_W));
	final_color = (final.r << 16) | (final.g << 8) | final.b;
	return (final_color);
}

static t_color	get_exit_color(t_data *data, t_color *exit_color,
		t_point *f_coords, t_img **ftex)
{
	int				ftex_x;
	int				ftex_y;
	...
	*ftex = data->texture->exit;
	...
	make_color_from_uint(floor_color, exit_color);
	return (*exit_color);
}
		...
		int *is_floor)
{
	...
	{
		get_exit_color(data, &exit_color, &f_coords, &ftex);
		if (exit_color.g > 0)
			ftex = data->texture->floor;
	}
	if (ftex)
	...
	*is_floor = (ftex == data->texture->floor);
	if (*is_floor == true && data->map_info->has_floor == false)
